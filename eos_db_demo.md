## EOS数据库操作
此文转自于eosfans，原帖：[https://eosfans.io/topics/484](https://eosfans.io/topics/484)
...
我将以上`.cpp`中的`das datable( _self, user);`改为`das datable( _self, _self);`.这样数
...
前几行代码之前讲解过，现在直接说`modify`方法，他的第一个参数是你查询出的要更改的对象，第二个参数是`payer`,其他的不用多说。
